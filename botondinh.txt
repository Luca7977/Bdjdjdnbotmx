...
function connect() {
  ...
  bot.on('spawn', () => {
    ...
    reportInterval = setInterval(() => {
      ...
      console.log(`📊 [${new Date().toLocaleTimeString()}] RAM: ${memUsageMB()}MB | Online: ${formatDuration(Date.now() - connectedSince)} | Pos: ${posStr}`)
    }, 15000)
  })
  ...
  function handleMessage(text) {
    ...
    if (/discord|liên kết/i.test(text))     { goIdle('Cần link Discord, không thể tiếp tục') }
  }
